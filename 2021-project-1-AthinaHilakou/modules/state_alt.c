...
struct state {
	...
	Map entrance_to_exit;		// περιέχει PortalPair (ζευγάρια πυλών, είσοδος/έξοδος)
	Map exit_to_entrance;
	struct state_info info;
};

// Ζευγάρια πυλών = εχει αντικατασταθεί από Map με portal pairs 
...
int compare_entrances_or_exits(Pointer a, Pointer b){ //συγκρίνει δύο εισόδους η εξόδους portal pair
	...
	while (node != SET_EOF){
		...
		if(enemy->forward){
			...
			if (node != NULL){	
				...
				//αφαίρεσε το τρέχον αντικείμενο από το set και ξαναβάλτο, με τις ενημερωμένες συντεταγμένες
...
void character_collision_control(State state){
	...
	if(character->forward){ 			//Αν ο Χαρακτήρας συγκρουστεί με μια Πύλη A ενώ κινείται προς τα δεξιά 
		if ((object = set_find_eq_or_greater(state->objects, temp_obj))!= NULL){
			free(temp_obj);		
			if (object->type == CHARACTER){ //σε περίτωση που το αμεσως μικρότερο στοιχειο του temp_obj είναι ο πραγματικός χαρακτήρας 
				SetNode temp_node =  set_find_node(state->objects, object); // δεν θέλουμε να ελέγξουμε σύγκρουση του character με τον εαυτό του
				temp_node = set_next(state->objects, temp_node);		//προχωράμε ακόμα μια θέση πίσω στο set 
				if (temp_node != NULL)
					object = set_node_value(state->objects, temp_node);	
				else 
					object = NULL; // αν χαρακτήρας βρίσκεται αριστερότερα από όλα τα στοιχεία του set 
			}
			if (object != NULL){
		...
		if ((object = set_find_eq_or_smaller(state->objects, temp_obj))!= NULL){
			...
			if (object->type == CHARACTER){ //σε περίτωση που το αμεσως μικρότερο στοιχειο του temp_obj είναι ο πραγματικός χαρακτήρας 
				SetNode temp_node =  set_find_node(state->objects, object); // δεν θέλουμε να ελέγξουμε σύγκρουση του character με τον εαυτό του
				temp_node = set_previous(state->objects, temp_node);		//προχωράμε ακόμα μια θέση πίσω στο set 
				if (temp_node != NULL)
					object = set_node_value(state->objects, temp_node);	
				else 
					object = NULL; // αν χαρακτήρας βρίσκεται αριστερότερα από όλα τα στοιχεία του set 
...
void enemy_collision_control(State state){
	...
	while(node!= SET_EOF){ // για κάθε εχθρό 
		...
		if(obj1->forward){ 			//Αν ο Χαρακτήρας συγκρουστεί με μια Πύλη A ενώ κινείται προς τα δεξιά 
			if ((obj2 = set_find_eq_or_greater(state->objects, temp_obj))!= NULL){	
				free(temp_obj);	
				if (obj2->type == ENEMY){ //σε περίτωση που το αμεσως μεγαλύτερο στοιχείο του temp_ememy είναι ο enemy ελεγξε και το επόμενο   
					SetNode temp_node =  set_find_node(state->objects, obj2); // δεν θέλουμε να ελέγξουμε σύγκρουση του character με τον εαυτό του
					temp_node = set_next(state->objects, temp_node);		//προχωράμε ακόμα μια θέση πίσω στο set 
					if (temp_node != NULL)
						obj2 = set_node_value(state->objects, temp_node);	
					else 
						obj2 = NULL; 
				}
